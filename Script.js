...
// =============================================
// FIREBASE - GERENCIAMENTO DE DADOS REAIS
// =============================================
    ...
    try {
        ...
        funcionariosRef.on('value', (snapshot) => {
            ...
            if (data) {
                ...
                // Banco vazio
                STATE.funcionarios = {};
                console.log('ℹ️ Banco de dados vazio');
                renderizarInterface();
                esconderLoading();
                mostrarNotificacao('Banco de dados vazio. Adicione funcionários.', 'info');
            }
...
async function adicionarFuncionarioFirebase(dados) {
    try {
        ...
        const novaChave = funcionariosRef.push().key;
        
        // Salvar dados
        await database.ref('funcionarios/' + novaChave).set({
            ...dados,
...
async function atualizarFuncionarioFirebase(funcionarioId, dados) {
    try {
        const database = firebase.database();
        await database.ref('funcionarios/' + funcionarioId).update({
            ...dados,
            dataAtualizacao: new Date().toISOString()
        });
...
async function excluirFuncionarioFirebase(funcionarioId) {
    try {
        const database = firebase.database();
        await database.ref('funcionarios/' + funcionarioId).remove();
        return true;
    } catch (error) {
